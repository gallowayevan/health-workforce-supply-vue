...
<g class="counties" :transform="`translate(${chartMargin.left}, ${chartMargin.top})`">
    <path v-for="item in countyFeatures" 
...
<g class="ahecs" :transform="`translate(${chartMargin.left}, ${chartMargin.top})`" v-show="aggregationLevel == 'ahec'">
    <path v-for="item in ahecFeatures" 
          :key="item.properties.county" 
          :d="generatePath(item.geometry)" 
          class="ahec"
...
export default {
  ...
  computed: {
    ...
    generatePath: function() {
      const chartWidth = this.width - this.chartMargin.left - this.chartMargin.right;
      const chartHeight = this.height - this.chartMargin.top - this.chartMargin.bottom;
    ...
    colorScale() {
      ...
      const numberOfColorBins = Math.max(Math.min(Math.round(currDomain.length/12), 6),3);
